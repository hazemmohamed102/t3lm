...
<?php
    ...
    if( $_SERVER["REQUEST_METHOD"] == "POST" ){
        ...
        if( empty($_REQUEST["heading"])){
            $heading_err =  "<p style='color:red'> * رأس المنشور مطلوب </p>";
        }else {
          $heading = $_REQUEST["heading"];
        }

        if( empty( $_REQUEST["detail"] ) ){
            $detail_err = "<p style='color:red'> * وصف المنشور مطلوب </p>";
        }else{
                        ...
                        if($add_post_by_1){
                            $heading = $detail = "";
                                    ...
                                    $(document).ready( function(){
                                        ...
                                        $('#linkBtn').text('إظهار جميع وصف المنشورات');
                                        $('#addMsg').text('!تم وضع وصف المنشور بنجاح');
                                        $('#closeBtn').text('اضف المزيد');
                                    })
                                </script>
                                ";
                        }
    ...
    <div id="form" class="pt-5 form-input-content">
        <div class="card login-form mb-0">
            <div class="card-body pt-3 shadow">
                <h4 class="text-center">إضافة وصف للمنشور </h4>
                <form method="POST" enctype="multipart/form-data" action=" <?php htmlspecialchars($_SERVER['PHP_SELF']) ?>"> 
                <div class="form-group">
                        <label >أختر رأس المنشور: </label>
                        <select name="heading" class="form-control" >
                            <option value="">رجاء اختر رأس المنشور: </option>
                            <?php
