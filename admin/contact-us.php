...
<?php 
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if(empty($_REQUEST["address"])){
            $address_err =  "<p style='color:red'> * العنوان مطلوب </p>";
            $address = "";
        }else{
        ...
        if( empty($_REQUEST["phn"]) ){
            $phn_no_err = "<p style='color:red'> * رقم الهاتف مطلوب </p>";
            $phn_no = "";
        }elseif(preg_match('/[a-zA-Z]/', $_REQUEST["phn"] ) ){ 
            $phn_no_err = "<p style='color:red'> * نسمح بالأرقام فقط</p>";
        ...
        if( empty($_REQUEST["email"])){
            $email_err = "<p style='color:red'> * الإيميل مطلوب </p>";
            $email = "";
        }elseif( !preg_match('/[@]/' , $_REQUEST["email"]) ){
            $email_err = "<p style='color:red'> * الإيميل خاطئ </p>";
        ...
        if( empty($_REQUEST["fax"]) ){
            $fax_err = "<p style='color:red'> * رقم الواتساب مطلوب </p>";
            $fax = "";
        }elseif(preg_match('/[a-zA-Z]/', $_REQUEST["fax"] ) ){ 
            $fax_err = "<p style='color:red'> * نسمح بالأرقام فقط</p>";
        ...
        if(!empty($email) && !empty($address) && !empty($phn_no) && !empty($fax)){
            ...
            if($result){
                ...
                $(document).ready( function(){
                    ...
                    $('#linkBtn').hide();
                    $('#addMsg').text('!تم تحديث معلومات الأتصال بنجاح');
                    $('#closeBtn').text('حسنا');
                })
             </script>
             ";
                    ...
                    <div class="form-input-content">
                        <div class="card login-form mb-0">
                            <div class="card-body pt-5 shadow">                       
                                    <h4 class="text-center pb-3">اضف وصف الأتصال</h4>
