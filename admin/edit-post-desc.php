...
<?php
    ...
    if( $_SERVER["REQUEST_METHOD"] == "POST" ){
        ...
        if( empty($_REQUEST["p_category"])){
            $category_err =  "<p style='color:red'> * التصنيف مطلوب </p>";
            $category = "";
        }else {
        ...
        if( empty( $_REQUEST["p_heading"] ) ){
            $p_heading_err = "<p style='color:red'> * رأس المنشور مطلوب </p>";
            $p_heading = "";
        }else{
        ...
        if( empty( $_REQUEST["editor"] ) ){
            $editor_err = "<p style='color:red'> * وصف المنشور مطلوب </p>";
            $editor = "";
        }else{
        ...
        if( empty($_FILES["thumbnail"]["name"])){
           $thumbnail_err = "<p style='color:red'> * صورة لمنشور مطلوبة </p>";
        }else{
         ...
         $thumbnail_name = $_FILES["thumbnail"]["name"];
          ...
          $location = "upload/thumbnail/".$new_file_name;
            ...
            $thumbnail_err = "<p style='color:red'> * فقط JPG , JPEG و PNG  ملفات مسموح بها </p>";
            $thumbnail_name ="";
          }
        }
        ...
        if( !empty($category) && !empty($editor) && !empty($p_heading) && !empty( $thumbnail_name) ){
            ...
            if($result_update_desc){
                ...
                $(document).ready( function(){
                    ...
                    $('#linkBtn').text('عرض كل المنشورات');
                    $('#addMsg').text('!تم تحديث معلومات المنشور بنجاح');
                    $('#closeBtn').text('تعديل مرة اخرى');
                })
             </script>
             ";
            }
    ...
    <div id="form" class="pt-5 form-input-content">
        <div class="card login-form mb-0">
            <div class="card-body pt-3 shadow">
                <h4 class="text-center">تعديل وصف المنشور </h4>
                <form method="POST" enctype="multipart/form-data" action=" <?php htmlspecialchars($_SERVER['PHP_SELF']) ?>"> 
                <div class="form-group">
                        <label >إختر تصنيف المنشور: </label>
                        <select name="p_category" class="form-control" >
                            <option value="">رجاء إختر تصنيف المنشور: </option>
                            <?php    
