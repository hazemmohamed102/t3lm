...
<?php
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if( empty($_REQUEST["about"]) ){
            $about_err = "<p style='color:red'>* لا تستطيع تركها فارغة</p>";
            $about = "";
        }else{
            ...
            if($insert_about_result){
                ...
                $(document).ready( function(){
                    ...
                    $('#linkBtn').hide();
                    $('#addMsg').text('!تم تحديث المعلومات بنجاح');
                    $('#closeBtn').text('!!حسنا');
                })
             </script>
             ";
    ...
    <div id="form" class="pt-5 form-input-content">
        <div class="card login-form mb-0">
            <div class="card-body pt-3 shadow">
                <h4 class="text-center">إضافة معلومات في قسم من نحن </h4>
                <form method="POST" enctype="multipart/form-data" action=" <?php htmlspecialchars($_SERVER['PHP_SELF']) ?>"> 
                       
                    <div class="form-group">
                        <label> <h4 class="pt-4">من نحن: </h4> </label>
